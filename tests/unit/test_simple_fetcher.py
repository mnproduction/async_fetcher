...
from toolkit.simple_fetcher import FetchResult, SimpleFetcher
...
class TestSimpleFetcher:
    ...
    async def test_fetch_single_with_cached_cookies(self, fetcher):
        ...
        mock_result = FetchResult(
            url="https://example.com", success=True, used_cookies=True, cookies_refreshed=False
        )

        with (
            patch.object(
                fetcher, "_fetch_with_cookies", AsyncMock(return_value=mock_result)
            ) as mock_fetch,
    ...
    async def test_fetch_single_needs_cookie_refresh(self, fetcher):
        ...
        with (
            patch.object(
                fetcher, "_fetch_with_cookies", AsyncMock(side_effect=mock_results)
            ) as mock_fetch,
            patch.object(fetcher.flaresolverr_client, "health_check", AsyncMock(return_value=True)),
            patch.object(
                fetcher.cookie_manager, "invalidate_domain", AsyncMock()
            ) as mock_invalidate,
        ):
            result = await fetcher.fetch_single("https://example.com")
    ...
    async def test_fetch_batch(self, fetcher):
        ...
        with patch.object(
            fetcher, "fetch_single", AsyncMock(return_value=mock_result)
    ...
    async def test_fetch_single_flaresolverr_unavailable(self, fetcher):
        ...
        with patch.object(
            fetcher.flaresolverr_client, "health_check", AsyncMock(return_value=False)
        ):
            result = await fetcher.fetch_single("https://example.com")
