...
class FlareSolverrClient:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.flaresolverr_url, timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
    ...
    async def create_session(self, session_id: str) -> bool:
        ...
        """
        payload = {"cmd": "sessions.create", "session": session_id, "maxTimeout": self.session_ttl}
    ...
    async def get_cookies_for_domain(
        self, url: str, session_id: str | None = None, timeout: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        solution = await self.solve_challenge(
            url=url, session_id=session_id, timeout=timeout, return_only_cookies=True
        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise FlareSolverrConnectionError(f"HTTP {response.status}: {error_text}")
