...
from .sanitization import sanitize_url, sanitize_html_content, sanitize_error_message, sanitize_url_list
...
class SingleFetchRequest(BaseModel):
    ...
    force_refresh_cookies: bool = Field(
        default=False,
        description="Force refresh of cached cookies even if they're still valid"
    )
...
class BatchFetchRequest(BaseModel):
    ...
    urls: List[str] = Field(
        ...
        min_length=1,
        max_length=100,
        description="List of URLs to fetch (1-100 URLs allowed)"
    )
    ...
    force_refresh_cookies: bool = Field(
        default=False,
        description="Force refresh of cached cookies for all URLs"
    )
...
class FetchResult(BaseModel):
    ...
    content: Optional[str] = Field(None, max_length=10_000_000, description="HTML content (success only)")
    ...
    error: Optional[str] = Field(None, max_length=1000, description="Error message (failure only)")
...
class HealthResponse(BaseModel):
    ...
    flaresolverr_healthy: bool = Field(..., description="Whether FlareSolverr service is accessible")
    cached_domains: int = Field(..., ge=0, description="Number of domains with cached cookies")
    cookie_sessions: Dict[str, Any] = Field(default={}, description="Detailed cookie session information")
    timestamp: float = Field(..., description="Health check timestamp")
    issues: Optional[List[str]] = Field(None, description="List of issues if status is not healthy")
